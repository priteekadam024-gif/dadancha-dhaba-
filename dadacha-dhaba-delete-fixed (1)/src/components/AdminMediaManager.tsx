...
export const AdminMediaManager: React.FC = () => {
  ...
  // Delete Media Item
  // Website media is deleted through the secure Render backend.
  const handleDeleteMedia = async (record: MediaFileRecord) => {
    if (!record.id) {
      showToast('This media item has no database ID.', 'error');
      return;
    }

    const confirmed = window.confirm(
      `Delete "${record.title || record.file_name || 'this media'}"? This will remove the database record and its uploaded file.`
    );
    ...
    try {
      ...
      const response = await fetch(
        `${apiBaseUrl}/api/admin/media/${encodeURIComponent(String(record.id))}`,
        {
          method: 'DELETE',
          headers: {
            'x-admin-token': adminToken,
      ...
      showToast(
        err?.message || 'Error deleting media',
        'error'
      );
    }
  };

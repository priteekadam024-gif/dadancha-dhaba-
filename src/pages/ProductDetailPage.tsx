...
export const ProductDetailPage: React.FC = () => {
  const { 
    ...
    currentUser, orders, isLoadingData
  } = useApp();
      ...
      <div className="max-w-7xl mx-auto px-4 py-16 text-center space-y-4">
        {isLoadingData ? (
          <div className="space-y-4 animate-pulse">
            ...
            <div className="h-64 bg-zinc-800 rounded-2xl max-w-lg mx-auto" />
          </div>
        ) : (
          <div className="bg-[#141414] border border-zinc-800 rounded-3xl p-12 max-w-lg mx-auto space-y-4">
            <p className="text-4xl">🍲</p>
            <h2 className="text-xl font-bold text-white font-marathi">
              {language === 'mr' ? 'उत्पादन सापडले नाही' : 'Product Not Found'}
            </h2>
            <button
              onClick={() => navigateTo('shop')}
              className="bg-[#F4B400] text-[#111111] font-bold text-xs px-6 py-2.5 rounded-full"
            >
              {language === 'mr' ? 'शॉप वर परत जा' : 'Back to Shop'}
            </button>
          </div>
        )}
      </div>
